...
func Execute() {
	...
	args := flag.Args()
	if len(args) < 1 {
		PrintUsage(os.Stderr, BaseCommand)
		return
	}
...
func SortCommands() {
	sort.Slice(BaseCommand.Commands, func(i, j int) bool {
		return SortLessFunc(BaseCommand.Commands[i], BaseCommand.Commands[j])
	})
}
